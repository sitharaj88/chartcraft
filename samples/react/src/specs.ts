/**
 * Northwind Cloud — chart specs.
 *
 * A PURE FUNCTION of (data, scheme), exactly as in the vanilla sample. Keeping
 * it out of the components is what lets `App` memoise the whole bundle on
 * `[data, scheme]`: every nested option object then keeps its identity across
 * re-renders, and `@chartcraft/react`'s per-key update effect stays quiet until
 * the theme or the range actually changes.
 *
 * `type` is deliberately NOT part of a spec — the typed components
 * (`<LineChart>`, `<SankeyChart>`, …) carry it, so a spec can only ever be
 * spread into the component that matches it.
 *
...
import type { ChartOptions } from '@chartcraft/react';

import { formatNumber, salesTerritories } from './data';
import type { DashboardData, KpiTile } from './data';

export type Scheme = 'light' | 'dark';

/** Everything a typed chart component needs — the component supplies `type`. */
export type ChartSpec = Omit<ChartOptions, 'type'>;

/** The keys are the inspector's chart ids, so the two can never drift apart. */
...
export const INSPECTABLE = {
  ...
  tickets: { title: 'Support load', format: (n: number) => `${formatNumber(n)} tickets` },
} as const satisfies Partial<Record<keyof DashboardSpecs, { title: string; format: (n: number) => string }>>;

export type InspectableId = keyof typeof INSPECTABLE;

/* ------------------------------------------------------------------ */

export function chartSpecs(d: DashboardData, mode: Scheme): DashboardSpecs {
...
export function sparkSpec(kpi: KpiTile, mode: Scheme): ChartSpec {
  return {
    theme: mode,
