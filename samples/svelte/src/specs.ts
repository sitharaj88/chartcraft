 ...
 *
 * Nothing here touches the DOM or Svelte: it is the same `chartSpecs()` idea
 * as the vanilla sample's `main.ts`, lifted into its own module so the
 * components stay markup. Because every spec is derived from the same inputs,
 * a re-render can never disagree with a first render — and because the specs
 * flow in as a plain `options` prop, `@chartcraft/svelte` routes them through
 * `chart.update()` rather than remounting: charts are created once and then
 * updated, which is what keeps a theme or range change animated instead of a
 * flash of empty cards.
 *
 * The per-type components (`<LineChart>`, `<GaugeChart>`, …) inject `type`
 * themselves, so these specs are `Omit<ChartOptions, 'type'>`.
 *
...
/** What the per-type components accept: full options minus the injected `type`. */
export type TypedOptions = Omit<ChartOptions, 'type'>;

/** One spec per card, keyed so the inspector can name its source. */
export interface ChartSpecs {
  mrr: TypedOptions;
  capacity: TypedOptions;
  flow: TypedOptions;
  products: TypedOptions;
  tickets: TypedOptions;
  segments: TypedOptions;
  territories: TypedOptions;
  contracts: TypedOptions;
}
...
export function sparkSpec(kpi: KpiTile, mode: Scheme): TypedOptions {
  return {
    theme: mode,
