<!--
  <DumbbellChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick />
  Same as <Chart> minus `type` (injected as 'dumbbell').
-->
<script>
...
<Chart
  bind:this={inner}
  options={withType(options, 'dumbbell')}
  class={className}
  on:pointclick
  on:pointenter
  on:pointleave
