<!--
  <PyramidChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'pyramid').
...
<Chart
  bind:this={inner}
  options={withType(options, 'pyramid')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
