<!--
  <BoxplotChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'boxplot').
...
<Chart
  bind:this={inner}
  options={withType(options, 'boxplot')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
