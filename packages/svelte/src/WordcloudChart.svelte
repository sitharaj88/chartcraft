<!--
  <WordcloudChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick />
  Same as <Chart> minus `type` (injected as 'wordcloud').
-->
<script>
...
<Chart
  bind:this={inner}
  options={withType(options, 'wordcloud')}
  class={className}
  on:pointclick
  on:pointenter
  on:pointleave
