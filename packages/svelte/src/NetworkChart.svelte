<!--
  <NetworkChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick />
  Same as <Chart> minus `type` (injected as 'network').
-->
<script>
...
<Chart
  bind:this={inner}
  options={withType(options, 'network')}
  class={className}
  on:pointclick
  on:pointenter
  on:pointleave
