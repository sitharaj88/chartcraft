<!--
  <WaterfallChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'waterfall').
...
<Chart
  bind:this={inner}
  options={withType(options, 'waterfall')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
