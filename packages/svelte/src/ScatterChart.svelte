<!--
  <ScatterChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'scatter').
...
<Chart
  bind:this={inner}
  options={withType(options, 'scatter')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
