<!--
  <ViolinChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick />
  Same as <Chart> minus `type` (injected as 'violin').
-->
<script>
...
<Chart
  bind:this={inner}
  options={withType(options, 'violin')}
  class={className}
  on:pointclick
  on:pointenter
  on:pointleave
