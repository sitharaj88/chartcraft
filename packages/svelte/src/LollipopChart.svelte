<!--
  <LollipopChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'lollipop').
...
<Chart
  bind:this={inner}
  options={withType(options, 'lollipop')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
