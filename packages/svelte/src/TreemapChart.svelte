<!--
  <TreemapChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'treemap').
...
<Chart
  bind:this={inner}
  options={withType(options, 'treemap')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
