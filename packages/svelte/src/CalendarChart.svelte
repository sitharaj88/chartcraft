<!--
  <CalendarChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'calendar').
...
<Chart
  bind:this={inner}
  options={withType(options, 'calendar')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
