<!--
  <BulletChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick />
  Same as <Chart> minus `type` (injected as 'bullet').
-->
<script>
...
<Chart
  bind:this={inner}
  options={withType(options, 'bullet')}
  class={className}
  on:pointclick
  on:pointenter
  on:pointleave
