<!--
  <ChoroplethChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick />
  Same as <Chart> minus `type` (injected as 'choropleth').
-->
<script>
...
<Chart
  bind:this={inner}
  options={withType(options, 'choropleth')}
  class={className}
  on:pointclick
  on:pointenter
  on:pointleave
