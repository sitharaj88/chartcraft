<!--
  <HistogramChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'histogram').
...
<Chart
  bind:this={inner}
  options={withType(options, 'histogram')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
