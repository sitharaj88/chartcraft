<!--
  <MarimekkoChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick />
  Same as <Chart> minus `type` (injected as 'marimekko').
-->
<script>
...
<Chart
  bind:this={inner}
  options={withType(options, 'marimekko')}
  class={className}
  on:pointclick
  on:pointenter
  on:pointleave
