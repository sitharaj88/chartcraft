<!--
  <SparklineChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'sparkline').
...
<Chart
  bind:this={inner}
  options={withType(options, 'sparkline')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
