<!--
  <CirclepackChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'circlepack').
...
<Chart
  bind:this={inner}
  options={withType(options, 'circlepack')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
