<!--
  <SankeyChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'sankey').
...
<Chart
  bind:this={inner}
  options={withType(options, 'sankey')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
