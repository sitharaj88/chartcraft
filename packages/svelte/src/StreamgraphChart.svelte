<!--
  <StreamgraphChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick />
  Same as <Chart> minus `type` (injected as 'streamgraph').
-->
<script>
...
<Chart
  bind:this={inner}
  options={withType(options, 'streamgraph')}
  class={className}
  on:pointclick
  on:pointenter
  on:pointleave
