<!--
  <SunburstChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick />
  Same as <Chart> minus `type` (injected as 'sunburst').
-->
<script>
...
<Chart
  bind:this={inner}
  options={withType(options, 'sunburst')}
  class={className}
  on:pointclick
  on:pointenter
  on:pointleave
