<!--
  <ParallelChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick />
  Same as <Chart> minus `type` (injected as 'parallel').
-->
<script>
...
<Chart
  bind:this={inner}
  options={withType(options, 'parallel')}
  class={className}
  on:pointclick
  on:pointenter
  on:pointleave
