<!--
  <IcicleChart {options} on:pointclick on:pointenter on:pointleave on:legendtoggle
    on:zoom on:annotationclick on:ready />
  Same as <Chart> minus `type` (injected as 'icicle').
...
<Chart
  bind:this={inner}
  options={withType(options, 'icicle')}
  class={className}
  {onpointclick}
  {onpointenter}
  {onpointleave}
